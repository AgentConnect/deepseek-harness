...
flowchart LR
  ...
  pkg_awiki["awiki"]
  svc_awiki["ctx.awiki<br/>Deployment-wide AWiki identity and messaging"]
  pkg_spill["spill"]
...
| `ctx.awiki` | `seam` | [`awiki`](../packages/awiki/awiki) | - | - | - | 该包持有 Service Definition、生产 TypeScript SDK provider 子路径，以及 Remote 与模型工具 Consumer；进程内每个 Agent 都使用同一个已注册 DID。 |
