...
| --- | --- | --- | --- | --- | --- |
| `@deepseek-ai/dsh-awiki` | `awiki_history`、`awiki_identity_status`、`awiki_list_conversations`、`awiki_send_attachment`、`awiki_send_message` | `ctx.tools`、`ctx.awiki`、`ctx.approval (send execution time)` | `tool/call`、`tool/result`、`external AWiki text or attachment after approval` | - | 三个读取工具只公开部署的公共状态。两个发送工具在执行时请求审批；五个工具都使用部署唯一的 AWiki 身份。 |
...
来源：[`packages/awiki/awiki/src/tools.ts`](../packages/awiki/awiki/src/tools.ts)

### `awiki_identity_status`

返回部署的 AWiki 身份状态；结果只包含公开 Handle 和 DID。
...
### `awiki_list_conversations`

列出部署 AWiki 身份的私聊和既有群聊会话。
...
### `awiki_send_attachment`

以部署 AWiki 身份上传并发送一个幂等附件；需要用户审批。
...
三个读取工具只公开部署的公共状态。两个发送工具在执行时请求审批；五个工具都使用部署唯一的 AWiki 身份。
