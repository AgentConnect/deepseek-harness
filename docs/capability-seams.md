...
| `ctx.awiki` | `seam` | [`awiki`](../packages/awiki/awiki) | - | - | - | The package owns the Service Definition, a production TypeScript SDK provider subpath, and Remote and model-tool Consumers; every Agent in the process uses the same registered DID. |
