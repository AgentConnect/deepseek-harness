...
| --- | --- | --- | --- | --- | --- |
| `@deepseek-ai/dsh-awiki` | `awiki_history`, `awiki_identity_status`, `awiki_list_conversations`, `awiki_send_attachment`, `awiki_send_message` | `ctx.tools`, `ctx.awiki`, `ctx.approval (send execution time)` | `tool/call`, `tool/result`, `external AWiki text or attachment after approval` | - | The three read tools expose only public deployment state. The two send tools request approval at execution time and all five use the deployment's single AWiki identity. |
...
Read one direct or group AWiki conversation history page.
...
Source: [`packages/awiki/awiki/src/tools.ts`](../packages/awiki/awiki/src/tools.ts)

### `awiki_identity_status`

Return the deployment AWiki identity status. The result contains only the public handle and DID.
...
List direct and existing group conversations for the deployment AWiki identity.
...
Upload and send one idempotent attachment as the deployment AWiki identity. User approval is required.
...
Send one idempotent text message as the deployment AWiki identity. User approval is required.
