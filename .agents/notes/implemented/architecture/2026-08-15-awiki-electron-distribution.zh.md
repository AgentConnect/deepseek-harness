...
macOS 发行凭据是全有或全无的构建输入。`Developer ID Application` 身份签名应用与 DMG，Apple ID 公证凭据提交并装订应用，CI 把 P12 导入临时钥匙串并在打包后删除。没有凭据时仍可执行本地和拉取请求验证构建；凭据不完整或使用非发行身份会直接失败。DMG 明确使用产品 ICNS 作为挂载后的卷宗图标。
