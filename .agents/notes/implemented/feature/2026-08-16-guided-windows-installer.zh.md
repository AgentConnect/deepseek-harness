...
桌面发行测试读取包 manifest（元数据清单）和 GitHub Actions 工作流，固定 Windows runner、上传的 EXE 路径、引导模式、目录选择、两类快捷方式、NSIS 命令和 Windows 图标路径。Windows job 验证构建的安装程序和内置的 `@awiki/dsh-plugin` 版本，将 EXE 大小和 SHA-256 写入运行摘要，并将上传的 artifact（产物）保留 14 天。
