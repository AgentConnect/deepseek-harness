...
Local builds remain unsigned when Apple credentials are absent. A release build signs every Mach-O file inside the compressed production runtime with hardened runtime and a secure timestamp before signing the app and DMG with the same `Developer ID Application` identity. It submits and staples the app, then separately submits the final DMG container, staples its ticket, and verifies both its Developer ID signature and Gatekeeper install assessment when `DSH_MACOS_SIGN_IDENTITY`, `APPLE_ID`, `APPLE_APP_SPECIFIC_PASSWORD`, and `APPLE_TEAM_ID` are all set; partial credentials fail the build. CI imports the password-protected P12 from `MACOS_CERTIFICATE_P12_BASE64` and `MACOS_CERTIFICATE_PASSWORD` into a temporary keychain and deletes that keychain after packaging. The certificate and private key belong in repository secrets, never in source control. Windows code signing, automatic updates, crash reporting, and replacing the loopback transport with a scoped IPC carrier remain release-hardening work.
