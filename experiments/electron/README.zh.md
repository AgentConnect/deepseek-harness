...
`Desktop installers` GitHub Actions 工作流在原生 runner 上运行相同命令。通过 `Run workflow` 可以手动构建安装包。Windows job 验证安装程序配置和内置的 `dsh-awiki` 版本，在运行摘要中发布 EXE 大小和 SHA-256，并将 EXE 作为 `deepseek-harness-windows-x64` artifact（产物）保留 14 天。
