...
这个 Electron 发行版把现有 `dsh web` 作为受控的本机回环子进程启动，并在启用 sandbox 的 Electron 窗口中展示。标准 Web profile 默认包含 `@awiki/dsh`，因此首次启动即可使用公开 `awiki.ai` 服务默认值访问 AWiki。
...
`Desktop installers` GitHub Actions 工作流在原生 runner 上运行相同命令。通过 `Run workflow` 可以手动构建安装包。Windows job 验证安装程序配置和内置的 `@awiki/dsh` 版本，在运行摘要中发布 EXE 大小和 SHA-256，并将 EXE 作为 `deepseek-harness-windows-x64` artifact（产物）保留 14 天。
